...
static PropertyFunction *solver3_region3_propfn(FREESTEAM_CHAR A){
	switch(A){
...
/* this function is a needed bit of a redundancy with solver3 in region 4 */
static double solver3_region4_p_Tx(double T, double x){
	(void)x;
	return freesteam_region4_psat_T(T);
}

static PropertyFunction *solver3_region4_propfn(FREESTEAM_CHAR A){
	switch(A){
		case 'p': return &solver3_region4_p_Tx;
...
static double solver3_region2_p_pT(double p, double T){
	(void)T;
	return p;
}

static PropertyFunction *solver3_region2_propfn(FREESTEAM_CHAR A){
	switch(A){
		case 'p': return &solver3_region2_p_pT;
...
/* completely unnecessary... */
static double solver3_region1_p_pT(double p, double T){
	(void)T;
	return p;
}

static PropertyFunction *solver3_region1_propfn(FREESTEAM_CHAR A){
	switch(A){
		case 'p': return &solver3_region1_p_pT;
...
#define D ((Solver3Data *)user_data)
	double rho = x[0];
	double T = x[1];
	f[0] = (*(D->Afn))(rho,T) - (D->a));
	f[1] = (*(D->Bfn))(rho,T) - (D->b));
...
SteamState freesteam_solver3_region3(FREESTEAM_CHAR A, FREESTEAM_CHAR B, double atarget, double btarget, SteamState guess, int *retstatus){
	...
	Solver3Data D = {A,B,solver3_region3_propfn(A), solver3_region3_propfn(B), atarget,btarget};
...
SteamState freesteam_solver3_region4(FREESTEAM_CHAR A, FREESTEAM_CHAR B, double atarget, double btarget, SteamState guess, int *retstatus){
	const gsl_multiroot_fdfsolver_type *T;
	gsl_multiroot_fdfsolver *s;
	int status;
	size_t iter = 0;
	...
	Solver3Data D = {A,B,solver3_region4_propfn(A), solver3_region4_propfn(B), atarget,btarget};
...
SteamState freesteam_solver3_region2(FREESTEAM_CHAR A, FREESTEAM_CHAR B, double atarget, double btarget, SteamState guess, int *retstatus){
	const gsl_multiroot_fdfsolver_type *T;
	gsl_multiroot_fdfsolver *s;
	int status;
	size_t iter = 0;
	...
	Solver3Data D = {A,B,solver3_region2_propfn(A), solver3_region2_propfn(B), atarget,btarget};
...
SteamState freesteam_solver3_region1(FREESTEAM_CHAR A, FREESTEAM_CHAR B, double atarget, double btarget, SteamState guess, int *retstatus){
	const gsl_multiroot_fdfsolver_type *T;
	gsl_multiroot_fdfsolver *s;
	int status;
	size_t iter = 0;
	...
	Solver3Data D = {A,B,solver3_region1_propfn(A), solver3_region1_propfn(B), atarget,btarget};
